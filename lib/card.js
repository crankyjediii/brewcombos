...
async function storyPNG(o, name, label) {
  ...
  const tree = h('div', { width: 1080, height: 1920, background: '#F8F8F2', color: '#243E35', padding: '250px 90px 320px', flexDirection: 'column', alignItems: 'center' },   // keeps clear of the story app's own buttons
    h('div', { fontFamily: 'Display', fontSize: 64, letterSpacing: -1 }, 'Brew Combos'),
    h('div', { fontFamily: 'Mono', fontSize: 30, letterSpacing: 3, color: '#647068', marginTop: 44 }, STORY_LABELS[label] || STORY_LABELS.mine),
    ...
    h('div', { width: 900, marginTop: 40, background: '#243E35', color: '#F8F8F2', borderRadius: 28, padding: '38px 44px', flexDirection: 'column' },
      h('div', { fontFamily: 'Mono', fontSize: 26, letterSpacing: 2.5, color: '#BBCDB8' }, 'SAY THIS AT THE WINDOW'),
      h('div', { fontFamily: 'Line', fontSize: line.length > 110 ? 44 : 52, lineHeight: 1.18, marginTop: 16 }, line)),
    h('div', { flexGrow: 1, minHeight: 56 }),
    h('div', { fontFamily: 'Body', fontSize: 34, color: '#647068' }, 'Find your drink at'),
    h('div', { fontFamily: 'Mono', fontSize: 56, color: '#CA4B2E', marginTop: 8 }, 'brewcombos.com'));
...
async function pinPNG(o, name, { label, date, why } = {}) {
  ...
  const line = M.orderLine(o);
  const cup = `data:image/svg+xml;base64,${Buffer.from(cupImageSVG(cupLook(o, M.flavorColor))).toString('base64')}`;
  const tree = h('div', { width: 1000, height: 1500, background: '#F8F8F2', color: '#243E35', padding: '84px 80px 80px', flexDirection: 'column', alignItems: 'center' },
    h('div', { fontFamily: 'Mono', fontSize: 28, letterSpacing: 3, color: '#CA4B2E' }, label === 'day' && date ? `DRINK OF THE DAY · ${date.toUpperCase()}` : '7 BREW DRINK COMBO'),
    ...
    ...(why ? [h('div', { fontFamily: 'Body', fontSize: 34, color: '#647068', marginTop: 18, textAlign: 'center', justifyContent: 'center', width: 800 }, why)] : []),
    { type: 'img', props: { src: cup, width: why ? 330 : 380, height: why ? 468 : 539, style: { marginTop: why ? 30 : 44 } } },
    h('div', { width: 840, marginTop: why ? 32 : 44, background: '#243E35', color: '#F8F8F2', borderRadius: 26, padding: '34px 40px', flexDirection: 'column' },
      h('div', { fontFamily: 'Mono', fontSize: 24, letterSpacing: 2.5, color: '#BBCDB8' }, 'SAY THIS AT THE WINDOW'),
    ...
    h('div', { width: 840, justifyContent: 'space-between', alignItems: 'baseline' },
      ...
      h('div', { fontFamily: 'Mono', fontSize: 30, color: '#CA4B2E' }, 'brewcombos.com')));
...
export async function cardPNG(o, name, { format, label, date, why } = {}) {
  ...
  const tree = h('div', { width: 1200, height: 630, background: '#F8F8F2', color: '#243E35', padding: '56px 72px', flexDirection: 'column', position: 'relative' },
    h('div', { justifyContent: 'space-between', alignItems: 'baseline', width: '100%' },
      ...
      h('div', { fontFamily: 'Mono', fontSize: 22, color: '#CA4B2E' }, 'brewcombos.com')),
    h('div', { fontFamily: 'Display', fontSize: titleSize, lineHeight: 0.9, letterSpacing: -1.5, marginTop: 40, width: 760 }, title),
    h('div', { position: 'absolute', left: 72, bottom: 56, width: 740, background: '#243E35', color: '#F8F8F2', borderRadius: 18, padding: '24px 28px', flexDirection: 'column' },
      h('div', { fontFamily: 'Mono', fontSize: 16, letterSpacing: 1.5, color: '#BBCDB8' }, 'SAY THIS AT THE WINDOW'),
