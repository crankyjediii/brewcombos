...
async function pinPNG(o, name) {
  ...
  const tree = h('div', { width: 1000, height: 1500, background: '#F5EFE6', color: '#221610', padding: '84px 80px 80px', flexDirection: 'column', alignItems: 'center' },
    h('div', { fontFamily: 'Mono', fontSize: 28, letterSpacing: 3, color: '#C8432B' }, '7 BREW DRINK COMBO'),
    ...
    { type: 'img', props: { src: cup, width: 380, height: 539, style: { marginTop: 44 } } },
    h('div', { width: 840, marginTop: 44, background: '#221610', color: '#F8EFE4', borderRadius: 26, padding: '34px 40px', flexDirection: 'column' },
...
export async function cardPNG(o, name, { format, label } = {}) {
  if (format === 'story') return storyPNG(o, name, label);
  if (format === 'pin') return pinPNG(o, name);
  const title = comboTitle(o, name);
  const line = M.orderLine(o);
  const cup = `data:image/svg+xml;base64,${Buffer.from(cupImageSVG(cupLook(o, M.flavorColor))).toString('base64')}`;
