...
export function orderFacts(o) {
  ...
  rows.push(['Caffeine', d.cat === 'nocaf' ? 'None' : 'Yes']);
  return rows;
}
...
export function orderVariations(o) {
  ...
  if (!o.sf && !o.extras.some(x => SF_SKIP.includes(x))) out.push(['Sugar-free', orderLine(fixCombo({ ...o, sf: true }))]);
...
export function applyPrefs(o, { size = 'drink', milk = 'drink', sf = false } = {}) {
  const d = DRINK[o.drink];
  const x = { ...o };
