...
// Bound recipe URLs, custom orders and share-image text to the same size.
...
export function cleanCustom(list) {
  ...
  return list
    ...
    .map(s => s.slice(0, 200).replace(/[^\p{L}\p{N} '&.-]/gu, '').replace(/\s+/g, ' ').trim().slice(0, 30))
    .filter(s => {
...
export function fixCombo(o) {
  ...
  const flavors = cleanCustom(o.flavors).map(name => FLAVORS.find(f => f.name.toLowerCase() === name.toLowerCase())?.name || name);
  const x = { ...o, drink: d.id, flavors: flavors.slice(0, MAX_FLAVORS), extras: Array.isArray(o.extras) ? [...o.extras] : [] };
  ...
  x.sf = !!x.sf;
  // Syrup type and how much syrup to request are independent choices.
  if (!SWEETS.some(w => w[0] === x.sweet)) x.sweet = 'regular';
  return x;
}
...
/* "What's in it" rows for a drink page. Flavors come back as a list for the caller to draw. */
export function orderFacts(o) {
  const d = DRINK[o.drink];
  const rows = [['Base', `${d.name} · ${d.desc.toLowerCase()}`], ['Size', cap(o.size)], ['Temperature', TEMP_LABELS[o.temp]]];
  if (d.milk === 'fixed') rows.push(['Milk', 'Half & half']);
  else if (o.milk && o.milk !== 'none') rows.push(['Milk', (MILKS[d.milk].find(m => m[0] === o.milk) || [, o.milk])[1]]);
  rows.push(['Flavors', o.flavors]);
  if (o.extras.length) rows.push(['Extras', o.extras.map(x => EXTRA[x].label).join(', ')]);
  if (o.sweet !== 'regular') rows.push(['Sweetness', SWEETS.find(w => w[0] === o.sweet)[2]]);
  if (o.sf) rows.push(['Syrups', 'Sugar-free']);
  rows.push(['Caffeine', d.cat === 'nocaf' ? 'No caffeine in base; check add-ins' : d.id === 'tea' ? 'Varies with tea choice' : 'Yes']);
  return rows;
}

/* "Other ways to order it": sugar-free, the other temperatures, large. */
export function orderVariations(o) {
  const d = DRINK[o.drink], out = [];
  if (!o.sf && !o.extras.some(x => SF_SKIP.includes(x))) out.push(['Ask for sugar-free syrups', orderLine(fixCombo({ ...o, sf: true }))]);
  for (const t of d.temps.filter(t => t !== o.temp)) out.push([TEMP_LABELS[t], orderLine(fixCombo({ ...o, temp: t }))]);
  if (o.size !== 'large') out.push(['Large', orderLine(fixCombo({ ...o, size: 'large' }))]);
  return out;
}

/* Apply someone's saved preferences (settings panel) to a combo: size, milk where the drink offers a choice, sugar-free. */
export function applyPrefs(o, { size = 'drink', milk = 'drink', sf } = {}) {
  const d = DRINK[o.drink];
  const x = { ...o };
  if (SIZES.includes(size)) x.size = size;
  if (milk !== 'drink') {
    if (d.milk === 'choice' && MILKS.choice.some(m => m[0] === milk)) x.milk = milk;
    // Cold brew: keep it black if it was black; otherwise swap in a non-dairy milk they asked for.
    if (d.milk === 'optional' && x.milk && x.milk !== 'none' && MILKS.optional.some(m => m[0] === milk)) x.milk = milk;
  }
  if (typeof sf === 'boolean') x.sf = sf;
  return fixCombo(x);
}
...
export function comboToQuery(o) {
  ...
  // Declare only custom flavors used in this recipe so a recipient can restore it
  // without needing the sender's local storage. Undeclared unknown names still drop.
  const custom = o.flavors.filter(name => !FLAVORS.some(f => f.name === name));
  if (custom.length) q.set('c', custom.join(','));
...
export function comboFromQuery(search, custom = []) {
  if (typeof search !== 'string' || search.length > 8192) return null;
  const q = new URLSearchParams(search);
  if (!DRINK[q.get('drink')]) return null;
  const list = k => (q.get(k) || '').split(',').slice(0, 100).map(s => s.trim()).filter(Boolean);
  const known = allFlavors([...cleanCustom(list('c')), ...cleanCustom(custom)]).map(f => f.name);
  return fixCombo({
    drink: q.get('drink'), temp: q.get('temp'), size: q.get('size'), milk: q.get('milk') || '',
    flavors: cleanCustom(list('f')).map(f => known.find(name => name.toLowerCase() === f.toLowerCase())).filter(Boolean).slice(0, MAX_FLAVORS),
...
- Coffee drinks, energy, tea, chai and matcha can have caffeine. Fizz, lemonade, smoothie and shake bases do not add coffee or energy; do not promise the finished drink is caffeine-free, especially with chocolate add-ins.
- Sugar-free syrup requests are not a guarantee that the finished drink is sugar-free. Milk, bases and toppings can still contain sugar, and syrup availability varies by stand.
...
export function userPrompt(vibe, kind, sf) {
  const limits = {
    ...
    nocaf: 'Use fizz, lemonade, smoothie or shake only. Avoid chocolate flavors and chocolate drizzle; do not guarantee the finished drink is caffeine-free.',
  }[kind] || 'Any drink type is fine.';
  const sfLine = sf ? ' Request sugar-free syrups where available (no drizzles, whip, soft top or cold foam). Keep requested sweetness separate from syrup type. Do not claim the complete drink has no sugar.' : '';
  return `Vibe: ${vibe}\n${limits}${sfLine}`;
}

const SF_SKIP = ['whip', 'caramel', 'chocolate', 'whitechoc', 'softtop', 'coldfoam'];
