...
- "why": one plain sentence linking the flavors to the vibe. No hype words.
...
export function normalizeCombo(c, { sf = false, custom = [] } = {}) {
  ...
  const combo = fixCombo({
    ...
    flavors,
    extras: (Array.isArray(c.extras) ? c.extras : []).map(x => String(x).toLowerCase().replace(/[^a-z]/g, '')),
    sf,
  });
...
export function parseCombos(text, opts) {
  ...
  const list = Array.isArray(data) ? data : data && data.combos;
